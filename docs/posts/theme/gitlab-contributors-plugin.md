 ...
 * @returns users
 */
export async function useAllUsers() {
  let users = [] as any[]
  let startPage = 1
 ...
 * @returns 指定项目的贡献者
 */
export function useRepositoryContributors(id: string) {
 ...
 */
export function useRepositoryCommits(projectId: string, path: string) {
 ...
 * @param path The file path
 */
export function useContributors(projectId: string, path: string) {
 ...
 * 获取当前页的贡献者
 */
export function usePageContributors() {
  return new Promise((resolve, reject) => {
...
export function contributorPlugin(options: ContributorOptions): PluginFunction {
  return app => {
